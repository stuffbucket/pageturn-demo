...
function sin2phiVert(pos: [number, number], crease: Crease): V3 {
  ...
  let flapPos: V3 = [rotated[0], originY + rotated[1], rotated[2]];
  ...
  if (pos[0] <= 1e-4) flapPos = [pos[0], pos[1], 0];

  // flapWeight smoothstep band — mirrors the FLIP_VERT shader. Without it
  // the test treated every point as 100% on the flap and counted vertices
  // that the actual shader leaves at rest as "rotated", artificially
  // inflating the stretch metric for tilted creases. With the smoothstep
  // band, off-flap points (s ≪ 0) blend back toward `pos`, matching what
  // the user sees on screen.
  const band = Math.max(flap, 1e-6) * 0.02;
  // smoothstep(-band, band, s)
  const tw = Math.max(0, Math.min(1, (s - -band) / (2 * band)));
  const fw = tw * tw * (3 - 2 * tw);
  return [
 ...
 *
 *  After the Option B fix (issue #78), the BookState passes a per-gesture
 *  spine anchor (the click's y at pointerdown) so the crease line's spine
 *  intersection cannot drift. For the corner-grab gesture characterised
 *  here the click sits at corner.y, so the anchor is corner.y.
 */
function dragStyleCrease(corner: Vec2, cursor: Vec2): Crease {
  return creaseFromDrag(corner, cursor, { x: W, y: H }, false, corner.y);
}
...
describe('Spine-strip stretch — drag vs settle uniform divergence', () => {
  ...
  for (const sc of scenarios) {
    ...
    // After the Option B fix (issue #78): with the per-gesture spine anchor
    // pinned at corner.y, the live drag path produces the same
    // inextensibility on the spine strip as the settle path. The earlier
    // `it.fails` polarity has been flipped to a plain `it` — this test now
    // pins the FIX rather than the bug.
    it(
